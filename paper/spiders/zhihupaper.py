...
class ZhihupaperSpider(Spider):
    ...
    def start_requests(self):
        # q_list = [41265507, 422158977, 421007203, 419858895, 418573946,
        #           417412940, 412638701, 395986277, 419999228, 424285865]
        # for question_id in q_list:
        ...
        yield Request(
            ...
            url.format(question_id=question_id, include=include), callback=self.parse, headers={'referer': 'https://www.zhihu.com/question/'+question_id}, dont_filter=True, errback=self.errback)
    ...
    def parse(self, response):
        ...
        if response['paging']['is_end'] == False:
            ...
            yield Request(next_url, callback=self.parse, errback=self.errback, dont_filter=True
                          )
        # else:
        #     print('■'*10+'爬取完毕'+'■'*10)
